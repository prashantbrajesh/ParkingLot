...
DB_FILE_PATH = os.path.join(config.DB_PATH, config.DB_NAME_VEHICLE)
...
class VehicleDao(object):
	...
	def insertIntoDb(self, jsonData):
		...
		sql = 'insert into '+ self.TABLE_NAME + ' (lotId ,time ,vehicleType,vehicleId) values ( %d, strftime("%%s", "now"),%s,%d)'%(lotId ,vehicleType,vehicleId)
	...
	def getParkings(self):
		...
		c = conn.cursor()
		sql =  'select lotId ,time ,vehicleType,vehicleId from ' + self.TABLE_NAME
	...
	def updateIndex(self, count):
		logging.info("Updating current index")
		c = self.conn.cursor()
		# print count
		sql3 =  'update %s set indexvalue = %d where name = "%s"'%( self.AUTO_INCRIMENT_TABLE ,count ,self.TABLE_NAME)
...
def main():
	...
	print ObjCustCount.getParkings()
